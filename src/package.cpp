...
QMap<QString, OutdatedPackageInfo> *Package::getOutdatedStringList()
{
  ...
  SDL2-2.0.3 <         Legacy SDL2 suitable for old Mac OS X versions
  */
  ...
  for(QString packageTuple: packageTuples)
  {
    QStringList parts = packageTuple.split(' ');
    {
      ...
      pNameVersion = parts[0];
      sep = pNameVersion.lastIndexOf("-");
      pkgName = pNameVersion.left(sep);

      opi.oldVersion = "";
      opi.newVersion = pNameVersion.mid(sep+1);
...
TransactionInfo Package::getTargetUpgradeList(const QString &pkgName)
{
  ...
  0 to remove, 0 to refresh, 0 to upgrade, 6 to install
  6678K to download, 33M of additional disk space will be used
  */
  ...
  {
    if (!infoTuple.contains(" "))
    {
      //0123456789
      //nameOfpkg-
      if (!infoTuple.trimmed().isEmpty())
      {
        sep = infoTuple.lastIndexOf("-");
        res.packages->append(infoTuple.left(sep));
      }
    }
    else if (infoTuple.contains("to download"))
    {
      QRegularExpression re("\\d+(B|K|M)");
      QRegularExpressionMatch match = re.match(infoTuple);
      bool hasMatch = match.hasMatch();
      if (hasMatch)
      {
        res.sizeToDownload = match.captured(0);
        //res.sizeToInstall = match.captured(1);
      }
    }
  }
...
{
  QFileInfo fi("/usr/pkg/pkgdb/" + pkgNameVersion + "/+COMMENT");
...
{
  QString pkgName, pkgOrigin, pkgVersion,
      pkgComment, pkgDescription, pkgInstallDate, pNameVersion;
  ...
  if(!pkgList.isEmpty())
  {
    ...
    for(QString packageTuple: packageTuples)
    {
      ...
      sep = pNameVersion.lastIndexOf("-");
      pkgName = pNameVersion.left(sep);
      pkgVersion = pNameVersion.mid(sep+1);
      ...
      pkgOrigin = "";
      //pkgComment = "";
      pkgInstallDate = getPackageInstallDate(pNameVersion);
      ...
      //qDebug() << "pkgName: " << pkgName << "PkgCommen: " << pkgComment;
...
{
  QString pkgName, pkgVersion, pkgCategories, pkgWWW, pkgComment, pNameVersion;
  int sep, pComment;
  //double pkgPkgSize;
  //int indName, indVersion, indCategories, indWWW, indComment, indPkgSize, sep;
  //const int cSpaces = 16;
  QList<PackageListData> * res = new QList<PackageListData>();

  /*
  py311-gdbm-3.11.7nb1 = Python interface to gdbm - GNU database manager
  ugdb-0.1.11          Alternative TUI for gdb
  ...
  */
  ...
  for(QString packageTuple: packageTuples)
  {
    ...
    if (packageTuple.at(0) == '=' ||
        packageTuple.at(0) == '<' ||
        packageTuple.at(0) == '>' ||
        packageTuple.contains("No results found")) continue;

    parts = packageTuple.split(QRegularExpression(" "), Qt::SkipEmptyParts);

    pNameVersion = parts[0];
    sep = pNameVersion.lastIndexOf("-");
    pld.name = pNameVersion.left(sep);

    if (packageCache.contains(pld.name)) continue;

    packageCache.append(pld.name);

    pld.version = pNameVersion.mid(sep+1);

    if (parts.at(1) == '=') pld.status = ectn_INSTALLED;
    else pld.status = ectn_NON_INSTALLED;

    if (pld.status == ectn_INSTALLED)
    {
      pld.installedOn = getPackageInstallDate(pNameVersion);
    }

    if (pld.status == ectn_NON_INSTALLED) pComment = 1;
    else pComment = 2;

    for(int c=pComment; c<parts.count(); c++)
    {
      pld.comment += " " + parts[c];
    }

    pld.comment = pld.name + " " + pld.comment.trimmed();
    pld.description = pld.comment;
    pld.repository = ctn_PKGNG_FAKE_REPOSITORY;

    //qDebug() << pld.name << pld.version << pld.status << pld.comment;
...
QStringList Package::getContents(const QString& pkgName, bool isInstalled)
{
  ...
  //Remove the first 2 rows fo the list
  fileList.removeAt(0);
  fileList.removeAt(0);
...
bool Package::hasPkgNGDatabase()
{
  ...
  if (!done)
  {
    if (UnixCommand::getBSDFlavour() == ectn_NETBSD)
    {
      QFile f(ctn_PKGNG_NETBSD_CORE_DB_FILE);
      answer = f.exists();
    }
