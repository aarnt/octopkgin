...
void MainWindow::doRemoveAndInstall()
{
  ...
  if(result == QDialogButtonBox::Yes || result == QDialogButtonBox::AcceptRole)
  {
    ...
    params << "-c";
    params << ctn_PKG_BIN + " -y remove " + listOfRemoveTargets + "; " +
              ctn_PKG_BIN + " -y install " + listOfInstallTargets;

    m_lastCommandList.clear();
    m_lastCommandList.append(ctn_PKG_BIN + " remove " + listOfRemoveTargets + ";");
    m_lastCommandList.append(ctn_PKG_BIN + " install " + listOfInstallTargets + ";");
    m_lastCommandList.append("echo -e;");
    m_lastCommandList.append("read -n1 -p \"" + StrConstants::getPressAnyKey() + "\"");
...
void MainWindow::doRemove()
{
  ...
  {
    QStringList params;
    QString command = ctn_PKG_BIN + " -y remove " + listOfTargets;

    m_lastCommandList.clear();
    m_lastCommandList.append(ctn_PKG_BIN + " remove " + listOfTargets + ";");
    m_lastCommandList.append("echo -e;");
    m_lastCommandList.append("read -n1 -p \"" + StrConstants::getPressAnyKey() + "\"");
...
void MainWindow::doInstall()
{
  ...
  if(result == QDialogButtonBox::Yes || result == QDialogButtonBox::AcceptRole)
  {
    ...
    QStringList params;
    QString command = ctn_PKG_BIN + " -y install " + listOfTargets;

    m_lastCommandList.clear();
    m_lastCommandList.append(ctn_PKG_BIN + " install " + listOfTargets + ";");
    m_lastCommandList.append("echo -e;");
    m_lastCommandList.append("read -n1 -p \"" + StrConstants::getPressAnyKey() + "\"");
...
{  
  ...
  */
  ...
  {      
    ...
    {
      QString altMsg = msg;
      writeToTabOutputExt(altMsg, ectn_DONT_TREAT_URL_LINK); //BLACK
    }
  }
...
void MainWindow::writeToTabOutputExt(const QString &msg, TreatURLLinks treatURLLinks)
{
  ...
  {      
    ...
    else
    {
               ...
               (!newMsg.contains(QRegularExpression("\\s"))))
      {
        newMsg = "<b><font color=\"#B4AB58\">" + newMsg + "</font></b>"; //IT'S A PKGNAME!
               ...
               (!newMsg.contains(QRegularExpression("http[s]:"))) &&
               (!newMsg.contains(QRegularExpression("->"))) &&
    ...
    if (msg.startsWith("calculating dependencies..."))
    {
      newMsg = "calculating dependencies...<br>";
    }
