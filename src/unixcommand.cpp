 ...
 * Overloaded with QString parameter
 */
QByteArray UnixCommand::performQueryAsRoot(const QString &args)
{
  QByteArray result("");
  QProcess pkg;
  ...
  params.insert(0, ctn_PKG_BIN);
  params.insert(0, ctn_OCTOPKG_DOAS_PARAMS);

  pkg.start(WMHelper::getSUCommand(), params);
  pkg.waitForFinished();
  result = pkg.readAllStandardError();
...
QByteArray UnixCommand::getRemotePackageList(const QString &searchString, bool useCommentSearch)
{
  ...
  result = performQuery("search " + searchString);
