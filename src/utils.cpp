...
{
  const QString ctn_NETBSD_RSS_URL = "http://www.netbsd.org/changes/rss-netbsd.xml";

  BSDFlavour bsd = UnixCommand::getBSDFlavour();
  QString res;
  QString tmpRssPath = QDir::homePath() + QDir::separator() + ".config/octopkgin/.tmp_distro_rss.xml";
  QString rssPath = QDir::homePath() + QDir::separator() + ".config/octopkgin/distro_rss.xml";
  QString contentsRss;
...
QString utils::parseDistroNews()
{
  ...
  if (bsd == ectn_NETBSD)
  {
    html = "<p align=\"center\"><h2>" + StrConstants::getNetBSDNews() + "</h2></p><ul>";
  }
